    ...
    class Navigator	
      ...
      def initialize(sections, options)
        ...
        if has_options?
          ...
          add_icons! if has_icons?
        end
      end
      
    private
      # checks to see whether we have advance options chosen for current selection
      def has_options?
      ...
      # add icons
      def add_icons!
        @sections.in_groups_of(3) { |group| ICONS[group.first] = group.last }
      end
      ...
      def fill_subtitles!
        @sections.in_groups_of(3) { |group| SUBTITLES[group.first] = group.second }
      end
      ...
      def parse(sections)
        returning(temp = []) do
          sections.each do |section|
            temp << section if section.is_a?(Symbol)
          end
        end
      end
      ...
      # ie. this is a reader method
      def sections
        has_options? ? parse(@sections) : @sections
      end
